...
public class CommonMessageViewHandler extends AbstractMessageHandler {
    ...
    public Message addMessageToModel(Map<String, Object> model, Long messageId, User user) throws IOException {
        ...
        if (message.getMsgState() == Message.MessageState.SENT && message.getLastStatus() != null) {
            if(this.isNotDelivered(message) && !message.getLocalParty().getBusiness().getName().equals("EDMA")) {
                model.put("statusCode", MessageListQueryParams.MessageStatus.UNDELIVERED.getMsgCode());
                model.put("statusDate", message.getLastStatus().getCreatedOn());
            } else {
