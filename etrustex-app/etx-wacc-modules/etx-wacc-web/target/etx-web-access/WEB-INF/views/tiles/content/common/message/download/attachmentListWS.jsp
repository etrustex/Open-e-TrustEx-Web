...
<!--WebStart attachmentListDownload -->
...
<script type="text/javascript" language="javascript"
        src="<c:url value="/js/download/signatureVerification.js?ver=${ver}"/>"></script>
<script type="text/javascript" language="javascript"
        src="<c:url value="/js/download/webstart/fileListModule.js?ver=${ver}"/>"></script>
                        ...
                        <span id="{{file.getAnchorId()}}" class="grid-row auto-height">
                            ...
                            <span class="grid-cell">
                                <img ng-if="file.comment != null"
                                     ...
                                     ng-click="!downloadCnt.isDisabled() && fileListCnt.showComment(file.comment)"
                                     title="{{file.comment}}"
                                     width="13" height="13"/>
                            </span>
                        </span>
...
<div class="" style="
    ...
    margin-top: 5px;
">
    <c:set var="downloadButtonStyle" scope="page" value="btn btn-primary"/>
    <c:if test="${retentionMetadata != null && retentionMetadata.retentionExpired}">
        <c:set var="downloadButtonStyle" scope="page" value="btn btn-primary.disabled"/>
    </c:if>

    <button type="button" id="openDownloadJNLP" class="${downloadButtonStyle}" download="downloadApplet.jnlp"><spring:message code="label.webstart.openDownloadWebstart"/></button>
</div>
...
<script type="text/javascript" language="javascript">
  $( document ).ready(function() {
    ...
    if (isRetentionExpired != 'true') {
      $('#openDownloadJNLP').click(donwloadJNLP);
    }

    function donwloadJNLP(){
      var a = document.createElement('a');
      document.body.appendChild(a);
      a.download = "downloadApplet.jnlp";
      var isTrusted=angular.element(document.getElementsByClassName("download-panel")).controller().isLocalCertificateTrusted();
      a.href ="applet/downloadApplet.do?sessionId=${pageContext.session.id}&msgId=${msg.id}&trusted="+isTrusted+"&token=${token}";
      a.click();
