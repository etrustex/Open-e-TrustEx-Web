...
<div class="content">
    <form:form id="messageListForm" method="post" commandName="formBean" action="${action}">
        <div class="content-controls">
                ...
                <c:choose>
                   ...
                   <span style="color:grey;" title="<spring:message code="error.ica.not.loaded"/>">
                    ...
                    <c:otherwise>
                        ...
                        <c:choose>
                            <c:when test="${userSessionContext.usingWebStart}">
                                <a href="<c:url value="/messageCreateWS.do?${ps.createMessageUrl}"/>">
                                    <spring:message code="label.new"/>
                                </a>
                            </c:when>
                            <c:otherwise>
            ...
            <spring:eval expression="@propertyConfigurer.getProperty('disableWebstart')" var="disableWebstart" />
            <c:if test="${!disableWebstart}">
                <c:choose>
                    <c:when test="${userSessionContext.usingWebStart}">
                        <a id="switchVersion" href="#">Switch to 3.2 (beta) version</a>
                    </c:when>
                    <c:otherwise>
                        <a id="switchVersion" href="#">Switch to 3.1 version</a>
                    </c:otherwise>
                </c:choose>
            </c:if>
        </div>
