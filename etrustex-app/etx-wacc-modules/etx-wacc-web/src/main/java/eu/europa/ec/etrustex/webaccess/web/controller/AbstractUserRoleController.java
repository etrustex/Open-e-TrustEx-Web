...
public class AbstractUserRoleController {
    ...
    protected Set<UserRoleXLS> transformToUserRoleXLS(Map<UserRole, BusinessUserConfig> map, Party party) {
        Set<UserRoleXLS> userRoleXLSs = new HashSet<>();
        for (Map.Entry<UserRole, BusinessUserConfig> entry : map.entrySet()) {
            UserRole userRole = entry.getKey();
            BusinessUserConfig businessUserConfig = entry.getValue();

            UserRoleXLS userRoleXLS = new UserRoleXLS();
            userRoleXLS.setPartyName(party.getDisplayName());
            userRoleXLS.setUserName(businessUserConfig.getName());
            userRoleXLS.setRoleType(userRole.getRole().getName().toUpperCase());
            userRoleXLS.setECASId(businessUserConfig.getUser().getLogin());
            userRoleXLS.setEmail(businessUserConfig.getEmail());
            userRoleXLS.setNotificationsEnabled(businessUserConfig.getNotificationsEnabled() ? "Yes" : "No");

            userRoleXLSs.add(userRoleXLS);
        }
        return userRoleXLSs;
    }
}
