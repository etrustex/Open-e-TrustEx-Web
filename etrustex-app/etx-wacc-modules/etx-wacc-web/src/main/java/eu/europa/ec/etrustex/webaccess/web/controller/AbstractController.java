    ...
    protected ModelAndView doGet(HttpServletRequest request) throws Exception {
        ...
        if (canSend) {
            if (modelAndView.getModel().get("hasLinkedIcas") != null && modelAndView.getModel().get("hasLinkedIcas").equals(0)) {
                if (getAction().getName().equals(WebAction.MESSAGE_CREATE.getName())) {
        ...
        String announcementsContent = party.getBusiness().getBusinessConfigValues().get(BusinessConfigProperty.BUS_ANNOUNCEMENT_CONTENT);
        announcementsContent = StringUtils.isBlank(announcementsContent) ? configurations.getAnnouncementsContent() : announcementsContent;
        modelAndView.addObject("announcementsContent", announcementsContent);
        modelAndView.addObject("zipSplitSize", configurations.getZipSplitSize());

        String archiveUploadSettings = party.getBusiness().getBusinessConfigValues().get(BusinessConfigProperty.BUS_FORBID_ARCHIVE_UPLOAD);
        if (StringUtils.isNotEmpty(archiveUploadSettings) && archiveUploadSettings.toLowerCase().equals("true")) {
            modelAndView.addObject("forbiddenUploadFileExtensions", configurations.getArchiveFilesExtensions());
        }
        ...
        String edmaSplashScreen = party.getBusiness().getBusinessConfigValues().get(BusinessConfigProperty.BUS_EDMA_SPLASH_SCREEN);
        modelAndView.addObject("edmaSplashScreen", edmaSplashScreen);

        if (modelAndView.getModel().get("hasMessageRemoteIca") != null && modelAndView.getModel().get("hasMessageRemoteIca").equals(0)) {
            logger.warn("No ICA available for selected Message: " + ((Message) modelAndView.getModel().get(MESSAGE_ATTR)).getId());
            return new ModelAndView(new RedirectViewWithLogging("error/error403.do", request, userSessionContext.getUser().getLogin()));
        }
        ...
        if (view.equals("edma") && getAction().getName().equals("messageCreate") && StringUtils.isNotEmpty(request.getParameter("mid") )) {
            ...
            Metadata metadata = attachmentHandler.getMetadata(messageId);
            EdmaMessage edmaMessage;
            if (metadata != null) {
                try {
                    edmaMessage = edmaUtils.convertXMLToEdma(metadata.getContent());
        ...
        modelAndView.addObject("compatibleBrowsers", objectMapper.writeValueAsString(compatibleBrowserService.getCompatibleBrowsers()));
