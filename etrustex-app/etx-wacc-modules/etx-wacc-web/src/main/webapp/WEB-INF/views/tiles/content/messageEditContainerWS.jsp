...
<script type="text/javascript" language="javascript" src="<c:url value="/js/upload/webstart/updateMessageIca.js"/>"></script>
...
<script>
    ...
    $(document).ready(function () {
        <c:choose>
            ...
            <c:otherwise>
                loadMessageIca("${formBean.icaDetailsVO}");
            </c:otherwise>
        </c:choose>

        $("#recipient").on('change', function(){
            updateMessageRecipient("${formBean.message.localParty.nodeName}", $("#recipient").val());
        })
    });
</script>
...
<form:form id="compMsg" method="POST" commandName="formBean" action="">
    ...
    <div class="content">
        ...
        <tiles:insertAttribute name="messageEditWS"/>
        ...
        <%-- <form:hidden path="encodedSignature"/> --%>
