...
export class BundleHandler {
  ...
  isReadyForSend(options) {
    ...
    return new Promise((resolve, reject) => {
      ...
      if (this.subject.trim().length == 0) {
        this.validationErrors.push('message.subjectErrorMessage')
        passed = false
      }
      if (this.filesInfo.size == 0) {
        this.validationErrors.push('message.attachmentListEmpty')
        passed = false
      }
      if(this.filesInfo.size > 500) {
        this.validationErrors.push('message.attachmentListTooLong')
        passed = false
      }
  ...
  isReadyForSave() {
    ...
    let readyForSavePromise = new Promise((resolve, reject) => {
      ...
      if (
        ...
        this.validationErrors.push('message.shouldBeSignedErrorMessage')
        passed = false
      }
      if (passed) {
        resolve()
  ...
  getTotalBytesToUpload(roundFunction) {
    ...
    this.toUpload.forEach((value, key, map) => {
      sum += roundFunction(value.file.size)
    })
    return sum
  }
  ...
  getSignableBundle() {
    ...
    let doc = document.implementation.createDocument('urn:eu:europa:ec:etrustex:signature:v1.0', null, null);
  ...
  sign(keysAndCert) {
    ...
    let signPromise = new Promise((resolve, reject) => {
      ...
      const privateKey = keysAndCert.keys.privateKey
      //const publicKey = keysAndCert.keys.publicKey
      const certificate = keysAndCert.certificate
      ...
      const pemCertificate = forge.pki.certificateToPem(certificate.cert, 80)
