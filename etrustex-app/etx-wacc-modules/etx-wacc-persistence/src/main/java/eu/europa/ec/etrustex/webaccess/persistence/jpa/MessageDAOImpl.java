...
public class MessageDAOImpl extends AbstractDAOImpl<Message, Long> implements MessageDAO {
    ...
    private final static String QL_COUNT_MESSAGE_BY_BUNDLE_ID = "SELECT COUNT(m.id) " +
            "FROM Message m " +
            "WHERE m.bundleId = :bundleId";
     ...
     */
    @Override
    public long countMessagesByBundleId(String bundleId) {
        Query totalMessagesCountQuery = entityManager.createQuery(QL_COUNT_MESSAGE_BY_BUNDLE_ID);
        totalMessagesCountQuery.setParameter("bundleId", bundleId);
        return (Long) totalMessagesCountQuery.getSingleResult();
    }
