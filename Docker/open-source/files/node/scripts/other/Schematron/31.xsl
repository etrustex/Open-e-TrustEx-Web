<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
 <xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
                 ...
                 xmlns:ec="ec:schema:xsd:RetrieveRequest-1"
 ...
 <xsl:template match="/">
       <svrl:schematron-output xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                               title="Business rules for e-TrustEx RetrieveRequest"
                               schemaVersion="">
          ...
          <svrl:ns-prefix-in-attribute-values uri="urn:oasis:names:specification:ubl:schema:xsd:DocumentStatusCode-1.0"
                                              prefix="stat"/>
          <svrl:ns-prefix-in-attribute-values uri="ec:schema:xsd:RetrieveRequest-1" prefix="ec"/>
          ...
          <svrl:active-pattern>
             <xsl:attribute name="id">RetrieveRequest_code_list_rules</xsl:attribute>
             <xsl:attribute name="name">RetrieveRequest_code_list_rules</xsl:attribute>
             <xsl:apply-templates/>
          </svrl:active-pattern>
    ...
    <!--SCHEMATRON PATTERNS-->
 <svrl:text xmlns:svrl="http://purl.oclc.org/dsdl/svrl">Business rules for e-TrustEx RetrieveRequest</svrl:text>
 
    <!--PATTERN -->
 
 
 	<!--RULE -->
 <xsl:template match="*[local-name()='RetrieveRequest']" priority="1000" mode="M10">
       <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                        context="*[local-name()='RetrieveRequest']"/>
 
 		    <!--ASSERT -->
 <xsl:choose>
          <xsl:when test="not ( normalize-space(cac:SenderParty/cbc:EndpointID) = '' )"/>
          <xsl:otherwise>
             <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                 test="not ( normalize-space(cac:SenderParty/cbc:EndpointID) = '' )">
                <xsl:attribute name="flag">error</xsl:attribute>
                <xsl:attribute name="location">
                   <xsl:apply-templates select="." mode="schematron-get-full-path"/>
                </xsl:attribute>
                <svrl:text>error.senderpartyid_check</svrl:text>
             </svrl:failed-assert>
          </xsl:otherwise>
       </xsl:choose>
       <xsl:apply-templates select="*" mode="M10"/>
    ...
    <!--PATTERN -->
 	...
 	<!--RULE -->
 <xsl:template match="*[local-name()='RetrieveRequest']" priority="1000" mode="M11">
       <svrl:fired-rule xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                        context="*[local-name()='RetrieveRequest']"/>
 
 		    <!--ASSERT -->
 <xsl:choose>
          <xsl:when test="not ( normalize-space(cac:ReceiverParty/cbc:EndpointID) = '' )"/>
          <xsl:otherwise>
             <svrl:failed-assert xmlns:svrl="http://purl.oclc.org/dsdl/svrl"
                                 test="not ( normalize-space(cac:ReceiverParty/cbc:EndpointID) = '' )">
                <xsl:attribute name="flag">error</xsl:attribute>
                <xsl:attribute name="location">
                   <xsl:apply-templates select="." mode="schematron-get-full-path"/>
                </xsl:attribute>
                <svrl:text>error.receiverpartyid_check</svrl:text>
             </svrl:failed-assert>
          </xsl:otherwise>
       </xsl:choose>
       <xsl:apply-templates select="*" mode="M11"/>
    ...
    <!--PATTERN RetrieveRequest_code_list_rules-->
