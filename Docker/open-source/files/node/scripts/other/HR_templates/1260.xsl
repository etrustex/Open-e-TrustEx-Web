		...
		<tbody>
			...
			<tr>
				<td colSpan="2" valign="top">
					<xsl:if test="count ( n1:CreditNoteReceived/mdcn:CreditNote/cac:AccountingCustomerParty/cac:Party/cac:PartyIdentification/cbc:ID) &gt; 0">
						<xsl:for-each select="n1:CreditNoteReceived">
							<xsl:for-each select="mdcn:CreditNote">
				...
				<td colspan="2" valign="top">
					<xsl:if test="count ( n1:CreditNoteReceived/mdcn:CreditNote/cac:AccountingSupplierParty/cac:Party/cac:PartyIdentification/cbc:ID) &gt; 0">
						<xsl:for-each select="n1:CreditNoteReceived">
							<xsl:for-each select="mdcn:CreditNote">
								<xsl:for-each select="cac:AccountingSupplierParty">
									<xsl:apply-templates select="cac:Party"/>
					...
					<xsl:variable name="CustomerID" select="normalize-space(/n1:CreditNoteReceived/eccac:ECDocumentReceivedData/eccbc:CustomerID)"/>
					<xsl:if test="count(document(&apos;NoLef.xml&apos;)//SimpleCodeList/Row/Value[.=$CustomerID]) = 0">
						<xsl:for-each select="/n1:CreditNoteReceived/eccac:ECDocumentReceivedData/eccbc:AdditionalID">
							<xsl:if test="./@schemeID=&apos;LEF&apos;"><b>LEF ID: </b><xsl:value-of select="."/><br/></xsl:if>
						</xsl:for-each>
					</xsl:if>
			...
			<tr>
				<td colspan="8">
					<table class="noBorder" border="1" cellPadding="2" cellSpacing="0" width="100%">
						<tbody>
							<tr class="bg2">
								<td class="labelBoldMedium" align="center" colspan="5">Lines description</td>
							</tr>
							<tr class="bg3">
